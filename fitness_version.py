...
def display_fitness_dashboard():
    ...
    workout_plan = st.session_state.fitness_coach.generate_workout_plan(st.session_state.user_data)

    # Storage the plan
    store_fitness_plan(workout_plan)

    # Workout plan section
    # st.markdown("<div class='card'><div class='card-header'>Your Workout Plan</div>", unsafe_allow_html=True)
    # Workout plan section title
    st.markdown("""
        <div style='background-color: #f0f4f7; padding: 20px; border-radius: 10px; margin-bottom: 20px;'>
            <h3 style='margin: 0; color: #2c3e50;'>🏋️ Your Personalized Weekly Workout Plan</h3>
            <p style='margin: 5px 0 0 0; color: #7f8c8d;'>Each plan is tailored to your fitness goal and availability ✨</p>
        </div>
    """, unsafe_allow_html=True)

    # get current plan
    current_plan = st.session_state.fitness_plan_storage[st.session_state.current_plan_id]

    # consuming
    total_weekly_calories = sum(day['total_calories'] for day in current_plan['weekly_plan'])
    st.success(f"🔥 Estimated Total Weekly Burn: {round(total_weekly_calories)} kcal")

    # display each day
    for day_plan in current_plan['weekly_plan']:
        day = day_plan['day']
    ...
    cur_plan = get_current_plan()
    meal_plan = st.session_state.nutritiest.generate_meal_plan(st.session_state.user_data, cur_plan)
...
def display_step3_goal_feasibility():
    ...
    timeline = target_months+1 if target_months >= realistic_months else realistic_months+1
...
def display_step4_generate_plan():
    ...
    if st.button("Generate My Plans!", type="primary", use_container_width=True):
        with st.spinner("Creating your personalized plans..."):
            ...
            st.session_state.fitness_coach.generate_workout_plan(st.session_state.user_data)
